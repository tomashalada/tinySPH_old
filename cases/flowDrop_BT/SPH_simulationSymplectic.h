...
struct SPH_simulation
{
	...
	()
	{
		...
		BZ_Integrate_outlet(particles, 2, dt);
		//BZ_Renew_outlet_buffer(particles, simulation_data, 1, dt);
	 ...
	 RemoveParticlesOutOfDomain(particles, simulation_data);
		...
		if(step%save_output_interval_interpolated == 0)
		{
			...
			GenerateInterpol(particles, simulation_data, output_file_nameInterpol, 0.1, 0., 1., 0.35);
