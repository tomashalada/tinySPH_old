...
#include "SPH_density_reinitialization.h"
...
struct SPH_simulation
{
	...
	//#include "constants.h"
	#include "constants_smallDeltaT.h"
	...
	()
	{
		...
		/* Apply BT, compute pressure */
		BT_compute_density_BTGeo(particles, simulation_data);
		Density_to_pressure(particles);
		std::cout << "SIMULATION -> RUN: Apply BT, compute pressure. DONE. " << std::endl;
	 ...
	 RemoveParticlesOutOfDomain(particles, simulation_data);
		...
		//if((step%40 == 0) && step > 2000)
		//{
		//Density_reinitialization_sumation(particles);
		//Density_reinitialization_overwrite(particles);
		//}
