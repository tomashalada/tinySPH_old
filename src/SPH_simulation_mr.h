 ...
 std::string output_file_nameHRfo = "resultsHRfo/particles_";
	...
	()
	{
		...
		if(step%save_output_interval == 0)
		{
			...
			output_file_nameLRfo += std::to_string(step) + ".vtk";
			output_file_nameHRfo += std::to_string(step) + ".vtk";
 	...
 	output_file_nameHRfo = "resultsHRfo/particles_"; //move
		...
		if(step == 5000)
		{
			...
			GenerateInterpol(particlesHR, simulation_dataHR, "interpolated/MESHEDhr", step, 0.865, 0., 1.7, 0.2);
			GenerateInterpol(particlesLR, simulation_dataLR, "interpolated/MESHEDlr", step, 0.2, 0., 0.88, 0.2);
