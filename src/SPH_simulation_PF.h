...
struct SPH_simulation
{
 ...
 std::string output_file_nameLR = "resultsLR_PF/particles_";
 std::string output_file_nameHR = "resultsHR_PF/particles_";
	...
	()
	{
		...
		Draw_geometry_dam_break_with_inlet_and_outletLR_PF(particlesLR, dpLR);
		Draw_geometry_dam_break_with_inlet_and_outletHR_PF(particlesHR, dpHR, dpLR);
		//Draw_geometry_dam_break_with_inlet_and_outletHR_PF(particlesHR, dpLR);
		...
		write_to_ASCII_VTK(particlesHR, "init_conditionHR.vtk");
		...
		Output_file(particlesHR.data_const, simulation_dataHR);
	...
	()
	{
		...
		//Renew_buffer(particlesLR, simulation_dataLR, LRoutletDyn_x0, LRinlet_y0, LRoutletDyn_xm, LRoutletDyn_ym, dpLR, step, cheat);
									...
									//Outlet_treatment_update_velocity(particlesLR, simulation_dataHR, 0.9-4*dpHR -dpHR/2, dpHR); // -dp/2 nDBC mirror line
																			...
																			//Renew_transition_zone_smooth(particlesHR, cheat, htz, 0.9 - 4*dpLR, 0.0 + dpHR, -1);
																			...
																			Integrate_inlet(particlesHR, dt, 0.9 - 4*dpLR, {0.0, 0.}, 4*dpHR); // buffer possitions???? Fixit
																			std::cout << "SIMULATION -> RUN: Second part integraion. DONE. " << std::endl;
		...
		if(step%save_output_interval == 0)
		{
			...
			output_file_nameHR += std::to_string(step) + ".vtk";
 	...
 	output_file_nameLR = "resultsLR_PF/particles_"; //move
 	output_file_nameHR = "resultsHR_PF/particles_"; //move
